...
Tracing is fully opt-in and lazy: if the OTel packages aren't installed, or
no collector endpoint is configured, ``setup_tracing()`` is a no-op — the
service must stay runnable in dev/CI without a collector.

Enable it by installing the ``otel`` extra (``uv sync --extra otel``) and
setting ``OTEL_EXPORTER_OTLP_ENDPOINT`` (e.g. ``http://localhost:4317``).
"""
...
def setup_tracing(app: FastAPI, service_name: str) -> None:
    ...
    logger.info(
        ...
        extra={"extra_fields": {"otlp_endpoint": endpoint, "service_name": service_name}},
    )
