...
- **SYS-05**: Enrollment captures **four head directions** (clock 12/3/6/9), not twelve. The browser-side estimator resolves the dominant axis and its sign reliably but not the yaw-to-pitch ratio, so it cannot place a pose inside a 30-degree sector — diagonals were consistently misclassified in live testing. Four targets 90 degrees apart need only the part the estimator gets right. **Accepted trade-off**: the gallery holds 4 pose buckets per subject instead of 12, narrowing multi-view coverage and some Recall headroom on extreme poses (FR-TRN-04's primary metric). The stored vocabulary stays 1..12 so the set can be widened later as configuration, without migration.
...
- **FR-ENR-02**: Frontend captures: (a) ≥1 frontal still photo, which also serves as the **neutral-pose reference** for QC (see FR-ENR-06); (b) a set of **still photos, one short burst (3–5 frames) per captured head direction**, taken automatically the moment the subject's live head orientation is detected there. The captured set is the **four cardinals — 12 (agak mendongak), 3 (agak menoleh ke kanan), 6 (agak menunduk), 9 (agak menoleh ke kiri)** (revised 2026-09-02, was all 12 clock positions; see SYS-05). The subject sweeps **head orientation** from 12 o'clock **clockwise** back to 12 o'clock (body stays facing the camera; only head yaw/pitch changes — see ASM-03, corrected 2026-08-30). ≥720p. Positions may be covered in any order and re-shot individually; a burst is kept so the embedding extractor still has multiple frames to average and pick sharpness from. *(Superseded 2026-09-02: previously one 10–20 s `video/webm` of the whole sweep. Sessions enrolled before this change keep their video, and both the backend contract and the ai-training pipeline continue to accept it — see TSD.md §7.)*
...
- **FR-ENR-08**: Enrollment requires recorded **consent** (who, when, consent text version) before capture can start. A grant is **recorded whenever it is offered on a live session**, not only while the session is `CREATED`: the operator flow moves a session to `CAPTURING` before the capture wizard opens, so gating the record on `CREATED` meant the consent the subject actually reads and accepts in the wizard — at the current clause version — was never stored. Only the state transition is gated. Re-granting a version already on record is a no-op (the wizard offers it on every camera start), and a terminal session (`CANCELLED`/`REVOKED`/`ENROLLED`) still refuses: consent for a revoked enrollment would make a revoked subject look freshly consented.
