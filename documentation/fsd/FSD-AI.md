...
- **FR-ENR-02**: Frontend captures: (a) ≥1 frontal still photo, (b) one video of the face starting at the 12 o'clock orientation, rotating **clockwise** a full turn back to 12 o'clock. Target duration 10–20 s, ≥720p, ≥24 fps. (See ASM-03 for the rotation-axis interpretation.)
- **FR-ENR-03**: Frontend MUST provide real-time guidance during capture: face-in-frame check, rotation progress indicator (clock positions covered), lighting/blur warnings, and retry flow.
...
- **FR-ENR-06**: A quality-check job (async) validates captured media: face detected in stills; in the video, sufficient pose coverage across yaw sectors (mapped from clock positions); sharpness/exposure thresholds. On failure the session becomes `REJECTED_QUALITY` with machine-readable reasons; the frontend prompts re-capture.
...
| ASM-03 | "360° rotation clockwise from 12 back to 12" is interpreted as the **head/body yaw rotating a full turn** (subject turns around), viewed from above; the usable face data is the ±90° yaw arc plus profile views — frames where the face is not visible (back of head) are sampled out. |
