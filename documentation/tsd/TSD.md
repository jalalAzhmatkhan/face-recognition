...
- Task: **1:N identification** (ASM-04). Gallery = multi-view embeddings per user from the head-orientation sweep (pose buckets keyed by (yaw, pitch) pairs derived from clock positions, up to 12 positions; the face stays camera-facing throughout — there is no back-of-head segment to discard; see ASM-03, corrected 2026-08-30). Buckets are filled from the per-position photo bursts, or, for sessions predating that change, from frames sampled out of the rotation video — both shapes converge on the same `frames_by_position` structure before embedding, so the extractor is unaware of which one it was fed.
